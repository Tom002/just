...
function readFileBytes (path, flags = fs.O_RDONLY) {
  ...
  let off = 0
  let len = net.read(fd, buf, buf.byteLength, off)
  while (len > 0) {
    off += len
    if (off === size) break
    len = net.read(fd, buf, buf.byteLength, off)
  }
  off += len
  r = net.close(fd)
  if (len < 0) {
...
function readFile (path, unused, flags = fs.O_RDONLY) {
  ...
  return sys.readString(buf, buf.byteLength, 0)
}
...
function copyFile (src, dest) {
  ...
  let bytes = net.read(source.fd, buf, buf.byteLength, 0)
  while (bytes > 0) {
    net.write(destination.fd, buf, bytes, 0)
    bytes = net.read(source.fd, buf, buf.byteLength, 0)
  }
  net.close(source.fd)
